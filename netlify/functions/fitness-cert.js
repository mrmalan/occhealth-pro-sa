...
async function generateCert(data) {
  return new Promise((resolve, reject) => {
    ...
    doc.font("Helvetica").fontSize(9).fillColor("#5DCAA5")
    ...
    doc.rect(m, 30*MM, W-2*m, 16*MM).fill(STATUS_COLORS[status] || TEAL);
    doc.fillColor(WHITE).font("Helvetica-Bold").fontSize(16)
       .text(STATUS_LABELS[status] || status.toUpperCase(), m, 35*MM, { align:"center", width: W-2*m, lineBreak:false });
    ...
    const fieldRow = (pairs) => {
      ...
      pairs.forEach((p, i) => {
        doc.fillColor(GRAY).font("Helvetica").fontSize(7)
           .text(p.label.toUpperCase(), m + i*colW, y, { lineBreak:false });
      });
      // Boxes + values
      pairs.forEach((p, i) => {
        const x = m + i*colW;
        doc.rect(x, y+3.5*MM, colW-2*MM, 7*MM).fill(FIELD_BG);
        doc.fillColor(DARK).font("Helvetica").fontSize(9)
           .text(p.value||"—", x+2*MM, y+5.5*MM, { width: colW-6*MM, lineBreak:false });
      });
      y += 14*MM;
    };
    ...
    fieldRow([{ label:"Full name", value:data.person_name }, { label:"Employee number", value:data.employee_number }]);
    fieldRow([{ label:"Employer", value:data.employer_name }, { label:"Job title / role", value:data.role_category }]);
    fieldRow([{ label:"Date of birth", value:data.date_of_birth }, { label:"Site / location", value:data.site }]);
    ...
    if (notes) {
      ...
      doc.fillColor(DARK).font("Helvetica").fontSize(10)
         .text(notes, m+3*MM, y+3*MM, { width: W-2*m-6*MM, height:18*MM });
      y += 26*MM;
    }
    ...
    fieldRow([{ label:"Full name", value:data.practitioner_name }, { label:"Qualification", value:data.qualification }]);
    fieldRow([{ label:"SANC number", value:data.sanc_number }, { label:"Date signed", value:data.signed_at }]);
    ...
    // Stamp box
    doc.rect(W-55*MM, y-20*MM, 40*MM, 22*MM).strokeColor("#E5E7EB").lineWidth(0.5).stroke();
    doc.fillColor(GRAY).font("Helvetica").fontSize(7)
