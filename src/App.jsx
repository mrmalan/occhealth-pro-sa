...
const CPD_STORAGE_KEY = "oh_cpd_activities";

const loadCPD = () => {
  try { return JSON.parse(localStorage.getItem(CPD_STORAGE_KEY) || "[]"); }
  catch { return []; }
};
...
const CPDTracker = ({ session, isCPDFree, onUpgrade }) => {
  ...
  const SANC_TARGET = 15;
  const year = new Date().getFullYear();
  const thisYearActs = activities.filter(a => a.date && a.date.startsWith(String(year)));
  const totalPoints = thisYearActs.reduce((s, a) => s + Number(a.points || 0), 0);
  const pct = Math.min(100, Math.round((totalPoints / SANC_TARGET) * 100));
  const remaining = Math.max(0, SANC_TARGET - totalPoints);

  const addActivity = () => {
    if (!form.date || !form.title || !form.points) { setFormErr("Date, activity name and points are required."); return; }
    const pts = parseFloat(form.points);
    if (isNaN(pts) || pts <= 0) { setFormErr("Points must be a positive number."); return; }
    const newItem = { id: crypto.randomUUID(), ...form, points: pts, createdAt: new Date().toISOString() };
    const updated = [newItem, ...activities].sort((a, b) => b.date.localeCompare(a.date));
    setActivities(updated);
    saveCPD(updated);
    setForm({ date: "", title: "", provider: "", category: CPD_CATEGORIES[0], points: "", notes: "" });
    setFormErr("");
    setShowAdd(false);
  };
  ...
  const exportPDF = () => {
    const meta = session?.user?.user_metadata || {};
    const name = meta.practitioner_name || "Occupational Health Practitioner";
    const sanc = meta.sanc_number || "";
    const rows = thisYearActs.map(a =>
      `<tr><td style="padding:6px 8px;border-bottom:1px solid #e5e7eb">${a.date}</td><td style="padding:6px 8px;border-bottom:1px solid #e5e7eb">${a.title}</td><td style="padding:6px 8px;border-bottom:1px solid #e5e7eb">${a.provider || "—"}</td><td style="padding:6px 8px;border-bottom:1px solid #e5e7eb">${a.category}</td><td style="padding:6px 8px;border-bottom:1px solid #e5e7eb;text-align:right;font-weight:600">${a.points}</td></tr>`
    ).join("");
    const html = `<!DOCTYPE html><html><head><title>CPD Log ${year}</title><style>body{font-family:Arial,sans-serif;padding:40px;color:#1a2e2a}h1{color:#0D6B6E;margin-bottom:4px}table{width:100%;border-collapse:collapse;margin-top:20px}th{background:#0D6B6E;color:#fff;padding:8px;text-align:left}td{font-size:13px}.total{font-size:16px;font-weight:700;color:#0D6B6E;margin-top:16px}.sig{margin-top:48px;border-top:1px solid #ccc;padding-top:16px;font-size:12px;color:#666}</style></head><body><h1>CPD Activity Log — ${year}</h1><p style="color:#444;font-size:13px">${name}${sanc ? " | SANC No: " + sanc : ""}</p><p style="font-size:13px;color:#444">SANC annual requirement: <strong>${SANC_TARGET} CPD points</strong> for Annual Practice Certificate renewal</p><table><thead><tr><th>Date</th><th>Activity</th><th>Provider</th><th>Category</th><th style="text-align:right">Points</th></tr></thead><tbody>${rows}</tbody></table><p class="total">Total points earned (${year}): ${totalPoints} / ${SANC_TARGET}${totalPoints >= SANC_TARGET ? " ✓ Requirement met" : " — " + remaining + " points remaining"}</p><div class="sig"><p>Signature: _________________________________ &nbsp;&nbsp; Date: _____________</p><p style="margin-top:8px">This log was generated by OccHealth Pro SA (occhealth-pro-sa.netlify.app)</p></div></body></html>`;
    const w = window.open("", "_blank");
  ...
  const barColor = pct >= 100 ? "#16a34a" : pct >= 60 ? "#d97706" : C.teal;
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.5rem" }}>
        <div>
          <h2 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: C.tealDark }}>CPD Tracker</h2>
          <p style={{ margin: "4px 0 0", fontSize: 13, color: C.muted }}>SANC requires 15 CPD points per year for Annual Practice Certificate renewal</p>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={exportPDF} style={{ padding: "8px 14px", borderRadius: 7, border: `1px solid ${C.teal}`, background: "#fff", color: C.teal, fontSize: 13, cursor: "pointer", fontWeight: 500 }}>
            {exported ? "✓ Opening PDF…" : "📄 Export PDF"}
          </button>
          <button onClick={() => setShowAdd(true)} style={{ padding: "8px 14px", borderRadius: 7, border: "none", background: C.teal, color: "#fff", fontSize: 13, cursor: "pointer", fontWeight: 600 }}>
            + Add activity
          </button>
        </div>
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: 12, border: `1px solid ${C.border}`, padding: "1.25rem", marginBottom: "1.25rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 10 }}>
          <div>
            <div style={{ fontSize: 13, color: C.muted, marginBottom: 2 }}>{year} progress</div>
            <div style={{ fontSize: 28, fontWeight: 700, color: barColor }}>{totalPoints}<span style={{ fontSize: 16, fontWeight: 400, color: C.muted }}> / {SANC_TARGET} points</span></div>
          </div>
          <div style={{ textAlign: "right" }}>
            {pct >= 100
              ? <span style={{ background: "#dcfce7", color: "#16a34a", borderRadius: 20, padding: "4px 12px", fontSize: 13, fontWeight: 600 }}>✓ Requirement met</span>
              : <span style={{ background: "#fef3c7", color: "#d97706", borderRadius: 20, padding: "4px 12px", fontSize: 13, fontWeight: 600 }}>{remaining} points to go</span>
            }
          </div>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8, fontSize: 12, color: C.muted }}>
          <span>{thisYearActs.length} activit{thisYearActs.length === 1 ? "y" : "ies"} logged this year</span>
          <span>APC renewal: March {year + 1}</span>
        </div>
      </div>

      {/* Category breakdown */}
      {thisYearActs.length > 0 && (
        <div style={{ background: "#fff", borderRadius: 12, border: `1px solid ${C.border}`, padding: "1rem 1.25rem", marginBottom: "1.25rem" }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: C.tealDark, marginBottom: 10 }}>Points by category</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {CPD_CATEGORIES.map(cat => {
              const pts = thisYearActs.filter(a => a.category === cat).reduce((s, a) => s + Number(a.points), 0);
              if (!pts) return null;
              return (
                <div key={cat} style={{ background: C.tealLight, borderRadius: 8, padding: "6px 12px", fontSize: 12 }}>
                  <span style={{ color: C.muted }}>{cat}: </span>
                  <span style={{ fontWeight: 700, color: C.tealDark }}>{pts} pts</span>
                </div>
              );
            })}
          </div>
      ...
      {showAdd && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.35)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "#fff", borderRadius: 14, padding: "1.75rem", width: 480, maxWidth: "95vw", boxShadow: "0 20px 60px rgba(0,0,0,0.18)" }}>
            <h3 style={{ margin: "0 0 1.25rem", color: C.tealDark, fontSize: 17 }}>Add CPD activity</h3>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div style={{ gridColumn: "1/-1" }}>
                <label style={{ fontSize: 12, color: C.muted, display: "block", marginBottom: 4 }}>Activity name *</label>
                <input value={form.title} onChange={e => setForm(f => ({...f, title: e.target.value}))} placeholder="e.g. SASOHN Annual Conference 2026" style={{ width: "100%", padding: "8px 10px", borderRadius: 7, border: `1px solid ${C.border}`, fontSize: 13, boxSizing: "border-box" }} />
              </div>
              <div>
              ...
              <div>
                <label style={{ fontSize: 12, color: C.muted, display: "block", marginBottom: 4 }}>CPD points *</label>
                <input type="number" min="0.5" max="15" step="0.5" value={form.points} onChange={e => setForm(f => ({...f, points: e.target.value}))} placeholder="e.g. 3" style={{ width: "100%", padding: "8px 10px", borderRadius: 7, border: `1px solid ${C.border}`, fontSize: 13, boxSizing: "border-box" }} />
              </div>
              <div>
                <label style={{ fontSize: 12, color: C.muted, display: "block", marginBottom: 4 }}>Provider / organiser</label>
                <input value={form.provider} onChange={e => setForm(f => ({...f, provider: e.target.value}))} placeholder="e.g. SASOHN, ETI, HPCSA" style={{ width: "100%", padding: "8px 10px", borderRadius: 7, border: `1px solid ${C.border}`, fontSize: 13, boxSizing: "border-box" }} />
              </div>
              <div style={{ gridColumn: "1/-1" }}>
                <label style={{ fontSize: 12, color: C.muted, display: "block", marginBottom: 4 }}>Category</label>
                <select value={form.category} onChange={e => setForm(f => ({...f, category: e.target.value}))} style={{ width: "100%", padding: "8px 10px", borderRadius: 7, border: `1px solid ${C.border}`, fontSize: 13, background: "#fff", boxSizing: "border-box" }}>
                  {CPD_CATEGORIES.map(c => <option key={c}>{c}</option>)}
                </select>
              </div>
              <div style={{ gridColumn: "1/-1" }}>
                <label style={{ fontSize: 12, color: C.muted, display: "block", marginBottom: 4 }}>Notes (optional)</label>
                <textarea value={form.notes} onChange={e => setForm(f => ({...f, notes: e.target.value}))} rows={2} placeholder="Certificate number, reflection notes, etc." style={{ width: "100%", padding: "8px 10px", borderRadius: 7, border: `1px solid ${C.border}`, fontSize: 13, resize: "vertical", boxSizing: "border-box" }} />
              </div>
            </div>
            {formErr && <div style={{ color: "#dc2626", fontSize: 12, marginTop: 8 }}>{formErr}</div>}
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: "1.25rem" }}>
              <button onClick={() => { setShowAdd(false); setFormErr(""); }} style={{ padding: "8px 16px", borderRadius: 7, border: `1px solid ${C.border}`, background: "#fff", fontSize: 13, cursor: "pointer" }}>Cancel</button>
              <button onClick={addActivity} style={{ padding: "8px 16px", borderRadius: 7, border: "none", background: C.teal, color: "#fff", fontSize: 13, cursor: "pointer", fontWeight: 600 }}>Save activity</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ background: "#fff", borderRadius: 12, border: `1px solid ${C.border}` }}>
        <div style={{ padding: "1rem 1.25rem", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: C.tealDark }}>Activity log</div>
          <div style={{ fontSize: 12, color: C.muted }}>All years — {activities.length} total</div>
        </div>
        {activities.length === 0 ? (
          <div style={{ padding: "3rem", textAlign: "center", color: C.muted }}>
            <div style={{ fontSize: 32, marginBottom: 12 }}>📚</div>
            <div style={{ fontWeight: 600, marginBottom: 4 }}>No activities logged yet</div>
            <div style={{ fontSize: 13 }}>Add your first CPD activity to start tracking towards your 15-point SANC requirement.</div>
          </div>
        ) : (
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "#f9fafb" }}>
                {["Date", "Activity", "Provider", "Category", "Points", ""].map(h => (
                  <th key={h} style={{ padding: "8px 12px", textAlign: h === "Points" ? "right" : "left", fontSize: 12, color: C.muted, fontWeight: 600, borderBottom: `1px solid ${C.border}` }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {activities.map(a => (
                <tr key={a.id} style={{ borderBottom: `1px solid ${C.border}` }}>
                  <td style={{ padding: "10px 12px", fontSize: 13, color: C.muted, whiteSpace: "nowrap" }}>{a.date}</td>
                  <td style={{ padding: "10px 12px", fontSize: 13, fontWeight: 500 }}>
                    {a.title}
                    {a.notes && <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>{a.notes}</div>}
                  </td>
                  <td style={{ padding: "10px 12px", fontSize: 13, color: C.muted }}>{a.provider || "—"}</td>
                  <td style={{ padding: "10px 12px", fontSize: 13, color: C.muted }}>{a.category}</td>
                  <td style={{ padding: "10px 12px", fontSize: 13, fontWeight: 700, color: C.teal, textAlign: "right" }}>{a.points}</td>
                  <td style={{ padding: "10px 12px", textAlign: "right" }}>
                    <button onClick={() => deleteActivity(a.id)} style={{ background: "none", border: "none", color: "#dc2626", cursor: "pointer", fontSize: 14, opacity: 0.6 }} title="Delete">✕</button>
                  </td>
                </tr>
              ))}
            </tbody>
